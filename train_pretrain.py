...
from lib.data.dataset_motion_3d import H36MDataset3D
...
def parse_args():
    ...
    parser.add_argument("--config", type=str, default="configs/multi_view/MB_ft_mv_h36m.yaml", help="Path to the config file.")
...
def evaluate(args, opts, model_pos, test_loader, datareader):
    print('INFO: Testing')
    results_all = []
    gt_all = []
    model_pos.eval()
    with torch.no_grad():
        for _, batch_input, batch_gt in tqdm(test_loader):
...
def train_epoch(args, model_pos, train_loader, losses, optimizer, has_3d, has_gt):
    model_pos.train()
    for idx, (batch_frame, batch_input, batch_gt) in tqdm(enumerate(train_loader)):    
        ...
        batch_seq = 0
        batch_cam = np.tile(np.arange(args.batch_size), (243, 1)).T
        optimizer.zero_grad()
    ...
    if not opts.evaluate:        
        ...
        for epoch in range(st, args.epochs):
            ...
            if args.no_eval:
                ...
                e1, e2, results_all = evaluate(args, opts, model_pos, test_loader, datareader)
