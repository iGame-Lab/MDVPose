...
df = pd.read_csv('/data/jiajun/MotionBERT/tools/data/AMASS/fps.csv', sep=',',header=None)  #read all the csv file
fname_list = list(df[0][1:])

processed_dir = '/data/jiajun/MotionBERT/tools/data/AMASS/amass_fps60/'
J_reg_dir = '/data/jiajun/MotionBERT/tools/data/AMASS/J_regressor_h36m_correct.npy'
all_motions = '/data/jiajun/MotionBERT/tools/data/AMASS/all_motions_fps60.pkl'

file = open(all_motions, 'rb')
motion_data = pickle.load(file) # len(motion_data) == 14244
J_reg = np.load(J_reg_dir)
all_joints = []

max_len = 2916                  # 2916 = 243 * 12
with open('data/AMASS/clip_list.csv', 'w') as f:
    ...
    for i, bdata in enumerate(motion_data):
        ...
        bm_fname = osp.join('/data/jiajun/MotionBERT/tools/data/AMASS/body_models/smplh/{}/model.npz'.format(subject_gender))
        dmpl_fname = osp.join('/data/jiajun/MotionBERT/tools/data/AMASS/body_models/dmpls/{}/model.npz'.format(subject_gender))
