...
from lib.utils.utils_data import my_split_clips_train, my_split_clips_test
random.seed(0)
    
class DataReaderH36M_MV(object):
    ...
    def get_split_id(self):
        if self.split_id_train is not None and self.split_id_test is not None:
            return self.split_id_train, self.split_id_test, 0, 0
        ...
        self.split_id_train, filename_train = my_split_clips_train(vid_list_train, self.n_frames, data_stride=self.data_stride_train) 
        self.split_id_test, filename_test = my_split_clips_test(vid_list_test, self.n_frames, data_stride=self.data_stride_test)
        return self.split_id_train, self.split_id_test, filename_train, filename_test
    
    def get_hw(self):
#       Only Testset HW is needed for denormalization
        test_hw = self.read_hw()                                    
        split_id_train, split_id_test, _ , __ = self.get_split_id()
    ...
    def get_sliced_data(self):
        ...
        split_id_train, split_id_test, filename_train, filename_test = self.get_split_id()
        ...
        # ipdb.set_trace()
        return train_data, test_data, train_labels, test_labels, filename_train, filename_test
