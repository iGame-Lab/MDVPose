...
def my_split_clips_test(vid_list, n_frames, data_stride):
    result = []
    n_clips = 0
    st = 0
    i = 0
    filename = []
    sub_name = -1
    vid_list_temp_name = vid_list[0][0:]
    while i<len(vid_list):
        i += 1
        if i==len(vid_list):
            break
        if (i-st == n_frames) and (vid_list[i] == vid_list[st]):
            result.append(range(st,i))
            if vid_list[i] == vid_list_temp_name:
                sub_name += 1
            else:
                sub_name = 0
                vid_list_temp_name = vid_list[i]
            formatted_sub_name = '{:02d}'.format(sub_name)
            if (vid_list[st][-4] == 'm'):
                filename.append(vid_list[st][0:11] + "_sub_" + formatted_sub_name + vid_list[st][-7:])
            else:
                filename.append(vid_list[st][0:21] + "_sub_" + formatted_sub_name + vid_list[st][-6:])
            st = st + data_stride
            n_clips += 1
        
        if vid_list[i]!=vid_list[i-1]: 
            st = i
    df = pd.DataFrame({'filename': filename, 'range': result})
    df = df.sort_values(['filename'], ascending=[True])
    result_ = df['range'].tolist()
    filename_ = df['filename'].tolist()
    return result_, filename_

def my_split_clips_train(vid_list, n_frames, data_stride):
    result = []
    n_clips = 0
    st = 0
    i = 0
    ...
    while i<len(vid_list):
        ...
        if (i-st == n_frames) and (vid_list[i] == vid_list[st]):
            ...
            if vid_list[i] in sub_name:
                sub_name[vid_list[i]] += 1
            else:
                sub_name[vid_list[i]] = 1

            formatted_sub_name = '{:02d}'.format(sub_name[vid_list[i]])
            if (vid_list[st][-4] == 'm'):
                filename.append(vid_list[st][0:11] + "_sub_" + formatted_sub_name + vid_list[st][-7:])
            else:
                filename.append(vid_list[st][0:21] + "_sub_" + formatted_sub_name + vid_list[st][-6:])
